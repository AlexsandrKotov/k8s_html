    ...
    stages {
        ...
        stage("Execute test command over SSH remote server"){
                steps{
                   ...
                   sh 'ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -T akotov@192.168.111.1;cd c:/kubernetes;kubectl set image deployment/k8shtml-deployment k8shtml=alexsandr/k8s_html:latest'
